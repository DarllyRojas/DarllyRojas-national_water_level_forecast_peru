...
//let ajax_url = 'https://senamhi.westus2.cloudapp.azure.com/geoserver/wfs?request=GetCapabilities';
    ...
    success: function (data) {
      if (!data.error) {
        ...
        /*
                ...
                $('#download_observed_water_level').removeClass('d-none');
                */
        ...
        $('#submit-download-simulated-bc-water-level').attr({
          target: '_blank',
          href:
            'get-simulated-bc-water-level-csv?' + jQuery.param(params_sim_bc),
        });

        $('#download_simulated_bc_water_level').removeClass('d-none');
  ...
  map.on('singleclick', function (evt) {
    if (map.getTargetElement().style.cursor == 'pointer') {
      ...
      if (wms_url) {
        ...
        $('#station-info').empty();
        $('#download_observed_water_level').addClass('d-none');
        $('#download_simulated_bc_water_level').addClass('d-none');
